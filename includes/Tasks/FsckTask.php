    ...
    public function execute() {
		...
		if ($this->hasOption(OPTION_IF_CONF_CHANGED)) {
			...
			if ($new_conf_md5 == $last_md5) {
				Log::log(INFO, "Skipping fsck; --if-conf-changed was specified, and the configuration file didn't change since the last fsck.");
				break;
			}
		}
		...
		DB::query("DELETE FROM tasks WHERE action = 'md5' $where_clause") or Log::log(CRITICAL, "Can't delete deprecated md5 tasks: " . DB::error());
		...
		DB::query($query) or Log::log(CRITICAL, "Can't update fsck_file/idle tasks to fsck_file/complete: " . DB::error());
		$result = DB::query("SELECT COUNT(*) AS num_updated_rows FROM tasks WHERE action = 'fsck_file' AND complete = 'yes' $where_clause") or Log::log(CRITICAL, "Can't find number of updated tasks for fsck_file/complete tasks: " . DB::error());
		...
		if ($row->num_updated_rows > 0) {
			...
			Log::log(INFO, "Will execute all ($row->num_updated_rows) pending fsck_file operations for $fsck_what_dir before running this fsck (task ID $task->id).");
			return;
		}

		Log::log(INFO, "Starting fsck for $fsck_what_dir");
		...
		if ($this->hasOption(OPTION_CHECKSUMS)) {
			...
			Log::log(DEBUG, "Spawned " . count($checksums_thread_ids) . " worker threads to calculate MD5 checksums. Will now wait for results, and check them as they come in.");
		}
			...
			if ($share_options !== FALSE) {
				...
				if ($this->hasOption(OPTION_METASTORE) !== FALSE) {
					if($metastore === FALSE) {
						...
						Log::log(DEBUG, "Starting metastores fsck for $subdir");
						foreach (get_metastores() as $metastore) {
						...
						Log::log(DEBUG, "Starting metastore fsck for $metastore/$subdir");
						gh_fsck_metastore($metastore, $subdir, $share);
					}
				}
				if ($storage_volume === FALSE && $this->hasOption(OPTION_ORPHANED)) {
					$subdir = "/$share" . str_replace($share_options['landing_zone'], '', $task->share);
					Log::log(DEBUG, "Starting orphans search for $subdir");
					foreach ($storage_pool_drives as $sp_drive) {
						if (StoragePoolHelper::is_drive_ok($sp_drive)) {
		...
		Log::log(INFO, "fsck for " . ($task->share == '' ? 'All shares' : $task->share) . " completed.");
		...
		if ($this->hasOption(OPTION_EMAIL)) {
			...
			Log::log(DEBUG, "Sending fsck report to $email_to");
