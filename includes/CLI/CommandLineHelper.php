...
class CommandLineHelper {
	...
	function __construct() {
		$this->cliCommandsDefinitions = array(
			'help'            => new CliCommandDefinition('help',             '?',   null,          null,                      "Display this help and exit."),
			'daemon'          => new CliCommandDefinition('daemon',           'D',   null,          'DaemonRunner',            "Start the daemon."),
			...
			'cancel-balance'  => new CliCommandDefinition('cancel-balance',   'B',   null,          'CancelBalanceCliRunner',  "Cancel any ongoing or scheduled balance operations."),
			...
			'status'          => new CliCommandDefinition('status',           'S',   null,          'StatusCliRunner',         "Display what the Greyhole daemon is currently doing."),
			'view-queue'      => new CliCommandDefinition('view-queue',       'q',   null,          'ViewQueueCliRunner',      "Display the current work queue."),
			'empty-trash'     => new CliCommandDefinition('empty-trash',      'a',   null,          'EmptyTrashCliRunner',     "Empty the trash."),
			'debug'           => new CliCommandDefinition('debug:',           'b:',  '=filename',   'DebugCliRunner',          "Debug past file operations."),
			'thaw'            => new CliCommandDefinition('thaw::',           't::', '[=path]',     'ThawCliRunner',           "Thaw a frozen directory. Greyhole will start working on files inside <path>. If you don't supply an option, the list of frozen directories will be displayed."),
			'wait-for'        => new CliCommandDefinition('wait-for::',       'w::', '[=path]',     'WaitForCliRunner',        "Tell Greyhole that the missing drive at <path> will return soon, and that it shouldn't re-create additional file copies to replace it. If you don't supply an option, the available options (paths) will be displayed."),
			'gone'            => new CliCommandDefinition('gone::',           'g::', '[=path]',     'GoneCliRunner',           "Tell Greyhole that the missing drive at <path> is gone for good. Greyhole will start replacing the missing file copies instantly. If you don't supply an option, the available options (paths) will be displayed."),
			'going'           => new CliCommandDefinition('going::',          'n::', '[=path]',      'GoingCliRunner',         "Tell Greyhole that you want to remove a drive. Greyhole will then make sure you don't loose any files, and that the correct number of file copies are created to replace the missing drive. If you don't supply an option, the available options (paths) will be displayed."),
			'replace'         => new CliCommandDefinition('replace::',        'r::', '[=path]',     'ReplaceCliRunner',        "Tell Greyhole that you replaced the drive at <path>."),
			'fix-symlinks'    => new CliCommandDefinition('fix-symlinks',     'X',   null,          'FixSymlinksCliRunner',    "Try to find a good file copy to point to for all broken symlinks found on your shares."),
			'delete-metadata' => new CliCommandDefinition('delete-metadata:', 'p:',  '=path',       'DeleteMetadataCliRunner', "Delete all metadata files for <path>, which should be a share name, followed by the path to a file that is gone from your storage pool. Eg. 'Movies/HD/The Big Lebowski.mkv'"),
			'remove-share'    => new CliCommandDefinition('remove-share:',    'U:',  '=share_name', 'RemoveShareCliRunner',    "Move the files currently inside the specified share from the storage pool into the shared folder (landing zone), effectively removing the share from Greyhole's storage pool."),
			'md5-worker'      => new CliCommandDefinition('md5-worker',       '',    null,          null,                      null),
			'getuid'          => new CliCommandDefinition('getuid',           'G',   null,          'GetGUIDCliRunner',        null),
		);
	...
	private function getActionCommand($command_line_options) {
		foreach ($this->cliCommandsDefinitions as $opt_name => $def) {
