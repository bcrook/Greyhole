...
class RemoveShareCliRunner extends AbstractCliRunner {
	...
	function __construct($options) {
		...
		if (!isset($shares_options[$this->share])) {
	...
	public function run() {
		global $storage_pool_drives, $config_file;
		...
		$storage_folders_with_files = array();
		$num_files_total = 0;
		foreach ($storage_pool_drives as $sp_drive) {
			if (!file_exists("$sp_drive/$this->share")) {
				$this->log("  Folder does not exist: $sp_drive/$this->share. Skipping.");
			} else {
				$storage_folders_with_files[] = "$sp_drive/$this->share";
				unset($result);
		...
		exec("/bin/sed -i 's/^.*num_copies\[".$this->share."\].*$//' " . escapeshellarg($config_file));
